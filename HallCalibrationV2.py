...
def scan_3D(filename):
    ...
    # Printing the magnetic field parameters
    print(str(B_max)+','+str(order),file = sourceFile)
    ...
    for i in range(len(l_list)): 
        ...
        flj.append(integrand1)
        
    flj = np.array(flj).transpose()
    legend = np.array([legend[i] for i in range(0,2*order,2)]).transpose()
    alk = np.matmul(np.linalg.inv(legend),flj).transpose()
    
    for ak in alk:
        for a in ak:
            print(a,end=',', file = sourceFile)
        print('', file = sourceFile)
...
def findB(filename,Vgiven,T):
    ...
    with open(filename+'Parameters.csv', 'r') as file:
        ...
        for row in read:
            ...
            # Scanning list of magnetic field parameters
            if i==0: 
                B_max = float(row[0]) 
                order = int(row[1])
